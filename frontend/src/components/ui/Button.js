...
export const Button = React.forwardRef(({ className, variant = 'primary', size = 'md', fullWidth = false, isLoading = false, disabled, children, ...props }, ref) => {
    ...
    return (_jsx("button", { ref: ref, disabled: disabled || isLoading, className: clsx(baseStyles, variants[variant], sizes[size], fullWidth && 'w-full', className), ...props, children: isLoading ? 'Loading...' : children }));
});
